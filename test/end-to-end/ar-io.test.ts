...
describe('ArIO', function () {
  it('should return the network contract info on the /info endpoint', async function () {
    ...
    assert.ok(res.data.bundlers);
    assert.ok(Array.isArray(res.data.bundlers));
    assert.ok(res.data.bundlers.length > 0);
    assert.ok(res.data.bundlers[0].url);
    assert.strictEqual(typeof res.data.bundlers[0].url, 'string');
  });
