...
-- updateFullyIndexedAt
    WITH FullyIndexedBundles AS (
    SELECT b.id
    FROM bundles b
             JOIN bundle_data_items bdi ON bdi.parent_id = b.id
    WHERE b.matched_data_item_count IS NOT NULL
      AND b.matched_data_item_count > 0
      AND b.last_fully_indexed_at IS NULL
      AND bdi.filter_id = b.unbundle_filter_id
    GROUP BY b.id, b.matched_data_item_count
    HAVING COUNT(*) = b.matched_data_item_count
)
UPDATE bundles
SET first_fully_indexed_at = COALESCE(first_fully_indexed_at, @fully_indexed_at),
    last_fully_indexed_at = @fully_indexed_at
WHERE id IN (SELECT id FROM FullyIndexedBundles);
