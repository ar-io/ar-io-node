...
INSERT INTO bundles (
  ...
  import_attempt_count = CASE
    WHEN @queued_at IS NOT NULL THEN
      COALESCE(import_attempt_count, 0) + 1
    ELSE
      import_attempt_count
  END
...
INSERT INTO new_data_item_tags (
  ...
  @root_transaction_id, @data_item_id, @data_item_tag_index,
  @height, @indexed_at
) ON CONFLICT (tag_name_hash, tag_value_hash, data_item_id) DO UPDATE SET
    height = COALESCE(EXCLUDED.height, new_data_item_tags.height);
...
INSERT INTO bundle_data_items (
  ...
  @indexed_at
                                   ...
                                   filter_id = COALESCE(EXCLUDED.filter_id, bundle_data_items.filter_id),
