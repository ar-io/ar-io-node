...
/**
 * Partitioned Rust CDB64 Writer - Creates prefix-partitioned CDB64 indexes
 * using the Rust-backed cdb64 library for better memory efficiency.
 *
 ...
 *
 * Uses the Rust CdbWriter which has ~4-5x better memory efficiency than the
 * TypeScript implementation due to tighter data packing.
 *
 * ## Output Structure
 * ```
 * output-dir/
...
/**
 * Options for creating a PartitionedRustCdb64Writer.
 */
export interface PartitionedRustCdb64WriterOptions {
 ...
 * Partitioned Rust CDB64 Writer - Creates prefix-partitioned CDB64 indexes.
 */
export class PartitionedRustCdb64Writer {
   ...
   */
  constructor(outputDir: string, options?: PartitionedRustCdb64WriterOptions) {
  ...
  add(key: Buffer, value: Buffer): void {
    ...
    if (this.partitions[partitionIndex] === null) {
      ...
      // Rust CdbWriter opens the file immediately in the constructor
      const writer = new CdbWriter(partitionPath);
    ...
    // Rust writer's put() is synchronous
    partition.writer.put(key, value);
  ...
  async finalize(): Promise<Cdb64Manifest> {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      if (partition !== null) {
        // Rust writer's finalize() is synchronous
        partition.writer.finalize();

        // Get file size after finalization
        const stats = await fs.stat(partition.filePath);
  ...
  async abort(): Promise<void> {
    // Rust CdbWriter doesn't have an abort method, but we can clean up the temp dir
    // The files will be deleted when we remove the temp directory
