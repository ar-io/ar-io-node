...
import { StreamingPartitionedCdb64Writer } from './streaming-partitioned-cdb64-writer.js';
import { Cdb64Reader } from './cdb64.js';
import { parseManifest, validateManifest } from './cdb64-manifest.js';

describe('StreamingPartitionedCdb64Writer', () => {
  let tempDir: string;
  ...
  describe('add', () => {
    ...
    it('should create scatter files (not CDB files) during scatter phase', async () => {
      ...
      const scatterFiles = await fs.readdir(path.join(tempDirPath, 'scatter'));
      assert.strictEqual(
