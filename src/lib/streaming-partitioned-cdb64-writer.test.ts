...
try {
  ...
  // Native cdb64 module not available - tests will be skipped
}

describe(
  'StreamingPartitionedCdb64Writer',
  { skip: !StreamingPartitionedCdb64Writer },
  () => {
    let tempDir: string;
    ...
    beforeEach(async () => {
      tempDir = await fs.mkdtemp(
        path.join(os.tmpdir(), 'streaming-partitioned-cdb64-test-'),
      );
    ...
    describe('open', () => {
      it('should create temp directory with scatter subdirectory', async () => {
        ...
        const scatterStats = await fs.stat(path.join(tempDirPath, 'scatter'));
        assert.strictEqual(scatterStats.isDirectory(), true);
    ...
    describe('add', () => {
      ...
      it('should create scatter files (not CDB files) during scatter phase', async () => {
        const writer = new StreamingPartitionedCdb64Writer(outputDir);
        await writer.open();
        ...
        // Scatter file should exist
        const scatterFiles = await fs.readdir(
          path.join(tempDirPath, 'scatter'),
        );
        assert.strictEqual(
          scatterFiles.some((f) => f === 'ab.scatter'),
          true,
        );

        // No CDB files in temp root
        const tempFiles = await fs.readdir(tempDirPath);
        assert.strictEqual(
          tempFiles.some((f) => f.endsWith('.cdb')),
          false,
        );
    ...
    describe('finalize', () => {
      ...
      it('should return valid manifest', async () => {
        ...
        assert.strictEqual(
          (partition00?.location as { type: 'file'; filename: string })
            .filename,
          '00.cdb',
        );
      });
      ...
      it('should clean up scatter files after build', async () => {
        const writer = new StreamingPartitionedCdb64Writer(outputDir);
        await writer.open();
        ...
        assert.strictEqual(
          files.some((f) => f === 'scatter' || f.endsWith('.scatter')),
          false,
        );
      });
    ...
    describe('progress reporting', () => {
      ...
      it('should call onBuildProgress during finalize', async () => {
        const progressCalls: {
        ...
        const writer = new StreamingPartitionedCdb64Writer(outputDir, {
          onBuildProgress: (partitionIndex, prefix, phase) => {
            progressCalls.push({ partitionIndex, prefix, phase });
          },
        });
        await writer.open();
        ...
        // Should have start/done pairs for each partition
        assert.strictEqual(progressCalls.length, 4);

        assert.deepStrictEqual(progressCalls[0], {
          partitionIndex: 0x00,
          prefix: '00',
          phase: 'start',
        });
        assert.deepStrictEqual(progressCalls[1], {
          partitionIndex: 0x00,
          prefix: '00',
          phase: 'done',
        });
        assert.deepStrictEqual(progressCalls[2], {
          partitionIndex: 0xab,
          prefix: 'ab',
          phase: 'start',
        });
        assert.deepStrictEqual(progressCalls[3], {
          partitionIndex: 0xab,
          prefix: 'ab',
          phase: 'done',
        });
    ...
    describe('getOutputDir', () => {
      it('should return the output directory path', () => {
        const writer = new StreamingPartitionedCdb64Writer(outputDir);
    ...
    describe('integration tests', () => {
      it('should handle many partitions with readable CDB output', async () => {
        const writer = new StreamingPartitionedCdb64Writer(outputDir);
        await writer.open();
