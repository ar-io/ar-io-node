    ...
    UnvalidatedChunkSource
{
  ...
  private async fetchUnvalidatedChunkFromArIOPeer(
    ...
    const log = this.log.child({ method: 'fetchUnvalidatedChunkFromArIOPeer' });

    try {
      log.debug('Fetching unvalidated chunk from AR.IO peer', {
        absoluteOffset,
      });
      ...
      for (let attempt = 0; attempt < retryCount; attempt++) {
        ...
        log.debug('Selected peers for unvalidated chunk retrieval attempt', {
          attempt: attempt + 1,
          peers: selectedPeers,
          totalPeers: this.peerManager.getPeerUrls().length,
        });
        ...
        for (const selectedPeer of selectedPeers) {
          ...
          try {
            ...
            log.debug('Failed to fetch unvalidated chunk from peer', {
              peer: selectedPeer,
              error: error.message,
              attempt: attempt + 1,
              responseTimeMs: responseTime,
